...
<body>
	...
	<div class="row">
		...
		<div class="col-md-4">
			...
			<input type="hidden" class="form-control" name="id_pengguna" id="Fullname" readonly="" value="{{ $topup->id_pengguna }}">
		
		<input type="hidden" class="form-control" name="tanggal" id="exampleInputEmail1" readonly="" value="{{ date('m/d/Y') }}">
		
		<input type="hidden" class="form-control" name="pembayaran" id="exampleInputEmail1" readonly="" value="{{$topup->pembayaran}}">
		
		<input type="hidden" class="form-control" name="id_pengguna" id="Fullname" readonly="" value="{{ $topup->jumlah }}">
	<div class="form-group">
		<label for="exampleInputEmail1">KODE GENERATE ANDA: </label>
		@php
		$kodeku = rand();
		@endphp
		<input type="text" class="form-control" name="pembayaran" id="exampleInputEmail1" readonly="" value="{{$kodeku}}">
	</div>
	<input type="hidden" class="form-control" name="kode" readonly="" value="{{ session()->get('nama') }}">
	{{csrf_field()}}
	<div class="text-center">
	<a href="{{url('transaksi/toupup/saldo').'/'.$topup->id.'/'.$topup->id_pengguna.'/'.$kodeku}}" class="btn btn-primary">SELESAI</a></div>
		</div>
		<div class="col-md-4"></div>
	</div>
@endif
